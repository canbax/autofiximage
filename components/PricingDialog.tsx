...
export const PricingDialog: React.FC = () => {
      ...
      case 'processing':
        return (
          <div className="flex flex-col items-center justify-center space-y-4 h-64">
            <Spinner />
            <p className="text-lg text-gray-700 dark:text-gray-300">{t('pricing.processing')}</p>
          </div>
        );
      case 'success':
        return (
          <div className="flex flex-col items-center justify-center space-y-4 h-64 text-center">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">{t('pricing.successTitle')}</h3>
            <p className="text-gray-600 dark:text-gray-400">{t('pricing.successMessage')}</p>
          </div>
        );
      case 'idle':
      default:
        return (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            <div className="border border-gray-200 dark:border-gray-700 rounded-lg p-6 flex flex-col">
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">{t('pricing.freePlan.title')}</h3>
              <p className="mt-2 text-gray-500 dark:text-gray-400">{t('pricing.freePlan.description')}</p>
              <div className="my-6">
                <span className="text-4xl font-bold text-gray-900 dark:text-white">$0</span>
                <span className="text-gray-500 dark:text-gray-400"> / {t('pricing.month')}</span>
              </div>
            ...
            <div className="relative border-2 border-indigo-500 rounded-lg p-6 flex flex-col">
              ...
              <h3 className="text-xl font-semibold text-gray-900 dark:text-white">{t('pricing.proPlan.title')}</h3>
              <p className="mt-2 text-gray-500 dark:text-gray-400">{t('pricing.proPlan.description')}</p>
              <div className="my-6">
  ...
  return (
    <div className="fixed inset-0 bg-gray-500 bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-80 z-40 flex items-center justify-center animate-fade-in" style={{ animationDuration: '0.2s' }}>
      <div
        className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-3xl m-4 relative border border-gray-200 dark:border-gray-700"
        role="dialog"
        aria-modal="true"
        aria-labelledby="pricing-dialog-title"
      >
        <button onClick={() => { closePricingDialog(); setPaymentState('idle'); }} className="absolute top-3 right-3 text-gray-400 hover:text-gray-800 dark:text-gray-500 dark:hover:text-white transition-colors z-10" aria-label="Close">
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
          </svg>
        </button>
        <div className="text-center">
            <h2 id="pricing-dialog-title" className="text-2xl font-bold text-gray-900 dark:text-white">{t('pricing.title')}</h2>
